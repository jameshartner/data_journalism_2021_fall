...
test <-read_rds
  as.tibble() %>%
  read_rds("ppp_slices/",files) %>%
  bind_rows()


#for loop that binds every single file in ppp_slices together
folder <- list.files("ppp_slices/")
for (files in folder) {
files <- read_rds() %>%
  bind_rows() %>%
  as.tibble()
...
# Create list of senate companies
senator_companies <- all_match_results$cleaned_asset_name

#This is just a test, wait until data is fully cleaned 
# create another list of senators name
senator_ownership<- all_match_results$senator_full_name
