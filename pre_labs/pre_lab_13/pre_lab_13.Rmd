...
**Answer**
I see a very messy linear regression plot. It seems that there are a lot of outliers that does not fit in the best fit line. 
```{r}
...
**Task** Run the following code. Describe the plot. What do you see?
**Answer**
This graph is messier than the previous graph. With the line of best fit, there are little to no points surrounding it so there is a very weak relationship between the two variables we are looking at. 
```{r}
...
**Answer**
There were three outputs from the datatable. The first output x value was pct_trump which had an NA y value since our y variable is pct_trump. The second value had a 52 percent for its y value and the last one had 22 percent for its y variable.
```{r}
reveal_data %>%
...
**Task** Run the 
following code. Describe the model output. What do you see?
**Answer**
our p-value is super low and our correlation is only at .5218 which is also pretty low. The correlation shows a weak positive relationship.
```{r}
...
**Answer**
This has an even worse correlation of .22 which is closer to 0 than .52. .22 shows an even weaker positive relationship.
```{r}
...
**Answer**
I see the mean of ppp_amount_per_employee and the mean_business_receiveing_ppe_percentages among the votes of Clinton and Trump during the 2016 Presidential Election.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**
This gave a us a two sample t-test with our t-value being roughly 1.22 and our p-value being .2295. We are able to calculate if our hypothesis is correct with the two means between each group and the degrees of freedom. Since our p-value is not less than .05 if we are doing a 95% confidence interval we don't reject our null hypothesis. So, we are getting closer.
```{r}
...
**Task** Run the following code. Describe the output. What do you see?
**Answer**
our p-value is very small. Our t-value is a large negative value, which can be good. We can find something with this though, since our p-value is very small, it can reject the null hypothesis which is more than likely "true difference in means between group Clinton and group Trump is equal to 0."
```{r}
