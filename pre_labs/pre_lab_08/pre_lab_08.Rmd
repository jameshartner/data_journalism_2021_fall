...
alldata %>%
  group_by(NAME, Population2000, Population2010) %>%
  summarize(
    largest=Population2010-Population2000
  ) %>%
...
**Answer**
Nevada had the largest percent growth. 
...
**Answer**
Howard County has the highest median household income for every county in Maryland. 
...
**Answer**
Montgomery has a way smaller margin of error for ACS estimate, but both counties fall in roughly 100000.
Is the difference between Howard and everyone else significant?
**Answer**
Howard has the highest ACS estimate at roughly 120,000 and no other county has that high, so yes, there is a difference. 
...
**Answer**
Somerset and Caroline county saw their median incomes fall. There is change for both of them since they have the minus sign in front of the columns.
```{r}
md10 %>%
...
**Answer**
I don't know why the data wasn't loading properly but I found zip codes 24001, 24510, 24039, 24019 and 24023 to have median household incoms of less than 50,000 dollars. 
