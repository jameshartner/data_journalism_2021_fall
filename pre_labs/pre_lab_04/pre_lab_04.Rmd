...
**Answer**
Yes, all three CSV population estimate files have the same column structure. All three files have nine columns and all three have the same column specifications.
```{r}
...
**Answer**
There are 195869 rows in the maryland_ppp file.
```{r}
...
**Answer**
There are 195869 rows in the file still. 
```{r}
...
**Answer**
The error message that I received was "Error: Can't join on `x$zip` x `y$zip` because of incompatible types. i `x$zip` is of type <character>>. i `y$zip` is of type <double>>." I think it means the column specifications for the zips are different on each file.'x$zip' (from file maryland_ppp_with_naics ) is a character specification whereas in 'yzip'(maryland_zcta file) the specification is a double specification. You can't combine different column specifications.
```{r}
...
**Answer**
No, there are some rows that have "NA" values in them, so some of the data from one file did not properly merge with the other file. My guess is due to some zip codes having nine digits in one file whereas in the other data file, all zip codes have only five digits. 
```{r}
maryland_ppp_with_naics_and_demographics <- maryland_ppp_with_naics %>% left_join(maryland_zcta, by=c("zip"="ZCTA5N"))
...
**Answer**
Yes, now every row has demographic data. 
```{r}
...
**Answer**
There were 44032 loans that went to ZCTAs with more than 50% non-Hispanic Black population. The average amount of these loans were 53836.46 dollars and the median amount was 20000 dollars. 
```{r}
...
**Answer**
The number of loans that went to ZCTAs with more than 50% non-hispanic white population for Maryland was 113937 loans. The average amount for these loans was $88140.24 and the median amount was 20833 dollars.
```{r}
...
**Answer**
For rural, there were 148 loans that went to ZCTAs with more than 50% non-Hispanic Black population. The rural average was 44328.87 dollars and the median was 19621 dollars. 
For urban, there were 43884 loans that went to ZCTAs with more than 50% non-Hispanic Black population. The rural average was 53868.53 dollars and the median was 20000 dollars. 
...
**Answer**
For rural, there were 18337 loans that went to ZCTAs with more than 50% non-Hispanic white population. The rural average was 73694.14 dollars and the median was 20833 dollars. 
For urban, there were 95600 loans that went to ZCTAs with more than 50% non-Hispanic white population. The rural average was 90911.14 dollars and the median was 20833 dollars. 
```{r}
