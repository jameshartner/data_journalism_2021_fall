...
**Answer**
I see a bunch of different files. 
```{r, echo=FALSE}
...
**Answer**
There are 33 columns and 7,581 rows in this data. Some columns are OBJECTD, ID, NAME, ADDRESS,etc.
```{r}
...
**Answer**
This filtered the hospital data that we just ran to only include all of the data regarding Maryland hospitals only. 
...
This summarized the type of hospitals and counted how many of those hospitals were founded in the dataset. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This filtered all of the Maryland hospitals with the type of General Acute Care, meaning only Maryland hospitals specializing in General Acute Care are here. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This put them in a scatterplot that looks like the state of Maryland. I'm assuming that all of the dots show where each hospital is located in Maryland. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This added the package tigris to my R library. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This displayed a dataset. In this dataset, there were 3233 rows and 18 columns
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This filtered the state of Maryland by Maryland's FP number. By filtering this, it grabs all of the counties of Maryland.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This gave a map of Maryland and where each county is located at. It did a really good job drawing it all out. 
```{r}
md_counties %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We basically added that previous filtered dataset of md_hospitals and mapped that image onto the image that we made in the previous task. We can see that upper north counties have like no hospitals but Baltimore City has tons of them. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This colored the dots on how many beds each hospital has. The darker the dot, the more beds that the hospital has. 
```{r}
...
**Answer**
This says that it is "attaching package: 'tidycensus'" and loaded the census_api_key. 
```{r}
library(tidyverse)
library(sf)
library(janitor)
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This filtered the hospital data and only took the Maryland hospitals with General Acute Care.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This summarized each the available bed amount in hospital by each Maryland county/independent city. Baltimore city had the most with 4505 total beds and Kent County only had 26.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This showed the census data for the population of each county/independent city of Maryland.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This pulled the latitude and longitude of each the 24 counties/independent city of Maryland.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This printed out the datasets that we have been working on. There are a few issues with these datasets since the names for the column counties don't match up so it might be hard to merge these two datasets. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This renamed the "NAME" column in md_county_population so we can merge with md_beds_by_county.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This merged both the datasets that we've been working on so we can map the amount of 100k people per bed by county.  However, it says there are NA beds in two of the counties, which shouldn't be right. The counties were Queen Anne's and Caroline. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We changed those two NA values to just 0. I'm assuming since there are no hospitals located in those counties, there won't be any hospital beds available.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We now added another column to find the ration of beds per 100k people for each county.
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
  mutate(total_beds = replace_na(total_beds,0)) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This plotted the work that we have just done with that dataset. The darker the blue means the less amount of beds that the respective county in Maryland has. 
```{r}
ggplot() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This changed the color a bit so we can actually see a better representation of the amount of hospital beds there are in each county.The lighter the color means that there are more hospital beds. However, it seems that a majority of Maryland counties have very little beds.
...
**Answer**
It seems that the amount of beds per 100k are very large in certain areas. What I mean by theis is that the surrounding counties around a county with a large ratio of hospital beds per 100k have very little beds per 100k. I also see that there are better ratios of bed per 100k people along the Eastern Shore compared to the rest of Maryland except for Baltimore City. I feel that is because even though they have less hospitals than central Maryland, the population is a lot smaller. 
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
