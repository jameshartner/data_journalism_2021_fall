...
https://datawrapper.dwcdn.net/5MZdY/1/
...
This tried to take all of the counties and summarize the number of loans per county. However, there are quite a few errors here. 
```{r}
...
**Answer** Describe what the code below is doing and the output.
This took the same thing but filtered out all of the rows that had less than 6 ppp loans.
```{r}
md_counties <- maryland_ppp %>%
    group_by(project_county_name) %>%
    summarise(count=n()) %>%
...
This attached each fips_code to their respective counties. 
...
This basically summarized all of the counts of loans in md_counties and merged it into md_flips. The data was cleaned previously. 
```{r}
...
Some of the counties are shaded in and Maryland is crooked. 
...
**Answer** Describe the changes from the previous map
WE've added more color to other counties and if you highlight over them, it showes the amount of loans there are per countie
...
https://datawrapper.dwcdn.net/KWepk/1/