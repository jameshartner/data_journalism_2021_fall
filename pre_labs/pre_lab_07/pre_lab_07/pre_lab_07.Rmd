...
**Answer**
After clicking the link, I was sent to the U.S Bureu of Labor Statistics website and their page on NAICS codes. This website gave an overview of the codes and what they meant in tables as well. 
### Task 3: Load image
...
**Answer**
The code broke down the website to show what categories made the website. There was the html list, the head, the body and the attachments of the website. 
```{r}
# read in the html
...
**Answer**
I see what the general page structure looks like. There are three different tags inside <'html'>. Those tags were <'head>, <'body'> and then the attachment section. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
By right clicking and hitting "inspect," I see a bunch of html code. To my knowledge, this is the html code that the table is built from. 
### Task 9: Load image
**Task** Run the following code to display an image showing what you should see on the web page.
```{r, echo=FALSE}
...
**Answer**
The outputs seem to be representing the tables from the webpage of the NAICS codes.  
```{r}
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
It brings us to what tables the webpage has. There are six tables but it's interesting because they're called "tibbles."
...
**Answer**
I see the tables that were on the webpage. Each table were represented separately in each dataframe. For example, the second table on the webpage was the second element.
### Task 14: Load image
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
We get the second table from the webpage. This table has the "sector" column and the description of each sector
### Task 16: Load image
...
**Answer**
This shows the second dataframe in full which is the second table on the webpage.
```{r}
...
**Answer**
This took out the last row from the previous dataframe, which was just the source. 
```{r}
# Read in all html from table, store all tables on page as nested list of dataframes.
naics_industry <- naics_url %>%
  read_html() %>%
...
**Answer**
This cleaned up the table that we loaded in the last chapter. We took away the last row and standardized the column headers. 
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
This is the "Industries at a Glance" webpage which discusses the Utilities Sector and the statistics of it. 
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer**
This broke down what was on the webpage. Very similar to what we did previously with the naics_industry. It also stored this into R. 
...
**Answer**
The output that appears is just the code of the table. We removed everything else in from the elements section of the webpage.
```{r}
...
**Answer**
The output shopws the dataframe being put in a datatable. The table itself is a little messy and needs to be cleaned. 
```{r}
...
**Answer**
We kept two columns of the dataset and only have one row. That row has data for the "employment, all employment..." dataframe. 
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
This page discusses NAICS 21, which is mining, quarrying, and oil and gas extraction. It has multiple tables discussing things related to this code like workplace trends. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
This shows the code NAICS 23 at a glance, which is the info page for construction. It is very similar to the page we discussed previously.
...
**Answer**
Each sector were printed. This was because all names were hardcoded into it. 
```{r}
...
**Answer**
This created a character value with a 1X10 matrix of all name sectors that we asked to list.
```{r}
...
**Answer**
This was able to combine the two previous codes and getting a name faster and easier than hardcoding. All of the names were printed like in task 14. 
```{r}
...
**Answer**
There was an error stating, "object 'sector_list' not found." This was because we named our list of sectors "list_of_sectors" and not "sector_list."
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**
The output was all of the names that we listed in "list_of_sectors."
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**
The output was an error stating, "object 'sector_name' not found." Similar to task 17, we can't have rename an object that we are trying to take out of in a for loop.
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer**
The for loop porinted us the numbers 1 through 10 in numerical order. 
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer**
This popped up the naics industry's name with the sector behind each name. 
```{r}
...
**Answer**
This showed how many rows were in the dataset of "naics_industry."
```{r}
nrow(naics_industry)
```
...
**Answer**
This printed out the number of rows in respect to naics_industry. It followed the similar process of being printed out numerically. No names were listed for the rows though.
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
...
**Answer**
There was only one row that appeared and the columns were the same from the original table.
```{r}
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer**
This showed the dataframe we made from the last chapter. It took the source url row away and kept the 20 other rows.
```{r}
# Define url of page we want to scrape
...
**Answer**
This added two more columns in the datatable that we have been working on. The new columns were "sector_url" and "sector_xpath_id."
```{r}
...
**Answer**
This changed up the ID from the previous table. 
```{r}
...
**Answer**
This deleted the Public Administration row from the data table.
```{r}
...
**Answer**
All rows were printed out seperately to their own seperate outputs. 
...
**Answer**
The output showed which URL matched which row. 
```{r}
...
**Answer**
This outputted the employee information for each naics_industry. The for loop took the name of those industries and matched them with the right employment information. 
```{r}
...
**Answer**
This cleaned up the tables and only allowed the row of "employement, all employees (seasonally adjusted)" to appear. 
...
for(row_number in 1:nrow(naics_industry)) {
    ...
    # Define id of table to ingest
    xpath_employment_table <- paste0('//*[@id="',each_row_df$url_xpath_id,'"]')
...
**Answer**
This added four more columns from the previous for loop (task 32).The new columns were "sector, description, sector_url, section_xpath_id."
```{r}
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer**
This cleaned up the tables to only show one row and three columns. The columns were "sector, description, jun_2021_employees."
```{r}
...
**Answer**
This formed all of the data tables from task 34 into one singular table. 
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
This wepage is the "Agriculture, Forestry, Fishing and Hunting," industry's page (NAICS)
### Task 39: Load image
**Task** Run the following code to display an image showing what you should see on the web page.
...
**Answer**
The Jun_2021_employees data has been removed for NAICS11. It is the same table as the cleaned one before, just no number for NAICS11 for the employees. 
```{r}
