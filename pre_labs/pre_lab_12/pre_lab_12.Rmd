...
author: "You already know who it is"
date: "11/09/2021"
...
**Answer**
There are 2 character rows and 11 dbl rows. This dataframe also shows us the county vaccinationsn by date, which is pretty interesting. 
```{r}
...
When we scroll down there is an NA variable. Evidently, there are 202 rows with the NA variable. 
```{r}
...
**Answer**
This took all of the NA county variable from the dataframe of county_vaccination_date. I feel that the NA variable represents people who don't live in the county. For example, people in West Virginia could have went and their vaccine in Hagerstown. Also, the NA data could also be people who live in another county but recieved their vaccine somewhere else. For example, I received my vaccine in Hagerstown but I live in Carroll County. 
...
Maybe those are faulty records? I feel that maybe the amount of doses were miscounted or they were the left over vaccine shots at the end of the day. 
```{r}
...
**Answer**
It seems that the rank of the last 7-10 days are on the lower spectrum, meaning less people are getting vaccine shots in Prince George's. Now, this could be a good sign, since the vaccination rate in PG County is so high, less and less people will get it per day. 
```{r}
...
**Answer**
The amount of vaccinations per day is going down. The dataset looked closed to a bell curve at first, with a spike around May-June. This is a good sign though, since there are less people that need a vaccine.
```{r fig.width=9}
ggplot() +
...
**Answer**
This shows the amount of Coronavirus cases that each county has reported since the start of the pandemic. It is at a constant rise becaue it seems to be taking a cumulative value from each county.
```{r}
...
**Task** Run the following code and describe what the resulting chart shows
**Answer**
This shows a better representation of the counties. From the first graph above, it seemed that some counties had no cases whatsoever. However, it clearly represents each reported case. 
...
**Answer**
There are 1923842 rows in the dataframe.
```{r}
...
**Answer**
There are now only 24 rows in this dataframe. 
```{r}
...
**Answer**
This shows us a better picture of the cumulative amount of covid cases that each county has reported. The lighter the color, the more cases the county has seen over the past year and a half. 
```{r}
ggplot() +
