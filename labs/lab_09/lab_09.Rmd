...
wv_graph %>%  
  ...
  geom_line(aes(x=date_approved, y=total_loans)) +
   ...
   labs(
    title="West Virginia Total Approved Loans in April 2020",
    x = "Date",
    y = "Total Loans",
    caption = "source: SBA PPP loan database"
  ) +
  geom_point(aes(x=date_approved, y=total_loans)) +
  geom_text(aes(x=date_approved, y=total_loans, label=total_loans), nudge_x=-1, nudge_y=-1, check_overlap=T)
...
**A2.** 
```{r}
state_data <- read_csv("data/state_ppp_all.csv") %>%
   mutate(state=case_when(state == "S Dakota" ~ "South Dakota",
   state=="N Dakota" ~"North Dakota", TRUE~state))



ACS_MASTER <-load_variables(2019, "acs5", cache = TRUE) 
ACS_jazz<- get_acs(geography= "State", variables="B01001_001", geometry = FALSE) %>%
  rename(state=NAME, population=estimate)

Joined_Dataset <- state_data %>% 
  left_join(ACS_jazz) %>%
...
Joined_Dataset %>%
  group_by(state) %>%
  summarize(
    total_loans_per_100k= ((total_ppp_loans/population)*100000)
  ) %>%
  arrange(desc(total_loans_per_100k)) %>%
  head(10) %>%
ggplot() +
  geom_bar(aes(x=reorder(state,total_loans_per_100k), weight=total_loans_per_100k), fill="blue") +
  coord_flip() + 
  labs(
    title="States with the Most PPP Loans per 100k",
    x = "total loans",
    y = "state",
    caption = "source: SBA PPP loan database"
  )
