...
**Answer**
maryland_ppp_top_counties is pulling the data from ppp_maryland and setting up as a new dataset. The new dataset will then show only project_county_names with the group_by function (we are grouping that dataset with that variable). We are then summarizing the datset and taking the count (total_loans) of the number of loans. This will count how many loans are in each county. By arranging, we are ordering the number of loans from lowest to highest, but desc is the opposite of that (so now the number of loans are arranged from highest to lowest). The head function will finally only take the top ten of the dataset.
```{r}
...
**Answer**
I see a blank screen since there has been nothing added for the ggplot. 
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
There is now a bar graph, which shows the number of loans for the top ten counties. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Answer**
This reordered the bars so it goes from lowest number of loans to highest. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
This transposed the graph, which means it flipped the x and y axis. This helped make the county names stand out better instead of being clumped together. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The background of the graph changed from grey to white. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Answer**
This installed ggthemes, so now we can have a proper background/theme for our data.
```{r}
...
**Answer**
The background is now blue instead of white.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
We have added a title, a caption anbd changed our x and y axis labels to have a better understanding of the data.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
I see the number of loans that were approved on certain dates. 
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
I see a line graph that seems to show the number of loans that were approved on each date. It shows that there was a huge spike roughly in April/May of 2020 for approved loans.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
The scale of x changed which helped give the x-axis better labeling. This will allow us to actually see what month that huge spike was in. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
This adjusted the x-axis labels so they're easier to read and not clumped up like in the previous task. It mainly turned the labels 45 degrees so they wouldn't be so clumped. 
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
